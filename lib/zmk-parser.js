...
const keyboard_constants_1 = require("./keyboard-constants");
/**
...
function normalizeLayout(layers) {
    ...
    layers.forEach((layer, layerIndex) => {
        ...
        if (layer.bindings.length !== keyboard_constants_1.TOTAL_KEYS) {
            throw new Error(`Layer "${layer.name}" has ${layer.bindings.length} bindings, expected ${keyboard_constants_1.TOTAL_KEYS}`);
        }
        // Map bindings to key positions
        // ZMK uses row-major order: [row0_col0, row0_col1, ..., row0_col11, row1_col0, ...]
        let bindingIndex = 0;
        for (let row = 0; row < keyboard_constants_1.ROWS; row++) {
            const keysInRow = keyboard_constants_1.ROW_KEY_COUNTS[row];
...
function parseKeymapFile(content) {
    try {
        ...
        const metadata = {
            ...
            version: '1.0.0',
            layout: 'ZMK',
            totalKeys: keyboard_constants_1.TOTAL_KEYS
        };
        return {
            layers,
            metadata
...
function validateKeymapFile(content) {
    ...
    try {
        ...
        layers.forEach((layer, index) => {
            if (layer.bindings.length !== keyboard_constants_1.TOTAL_KEYS) {
                errors.push(`Layer ${index} ("${layer.name}") has ${layer.bindings.length} bindings, expected ${keyboard_constants_1.TOTAL_KEYS}`);
            }
