...
const ZMK_KEY_LABELS: Record<string, string> = {
    'TAB': '⇥',
    'BSPC': '⌫',
    'DEL': '⌦',
    'ESC': '⎋',
 ...
 */
export function parseBinding(zmkBinding: string): Binding {
 ...
 */
function extractLayers(content: string): Array<{ name: string; bindings: string[] }> {
    const layers: Array<{ name: string; bindings: string[] }> = [];
 ...
 */
export function normalizeLayout(layers: Array<{ name: string; bindings: string[] }>): Layer[] {
    ...
    layers.forEach((layer, layerIndex) => {
        ...
        if (layer.bindings.length !== TOTAL_KEYS) {
            throw new Error(
                `Layer "${layer.name}" has ${layer.bindings.length} bindings, expected ${TOTAL_KEYS}`
            );
        }
        ...
        for (let row = 0; row < ROWS; row++) {
            const keysInRow = ROW_KEY_COUNTS[row as keyof typeof ROW_KEY_COUNTS];
 ...
 */
export function parseKeymapFile(content: string): KeymapConfig {
    try {
 ...
 */
export function validateKeymapFile(content: string): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    try {
        ...
        layers.forEach((layer, index) => {
            if (layer.bindings.length !== TOTAL_KEYS) {
                errors.push(
                    `Layer ${index} ("${layer.name}") has ${layer.bindings.length} bindings, expected ${TOTAL_KEYS}`
                );
            }
 ...
 */
export function getSupportedBindings(): string[] {
    return [
        '&kp KEY - Keypress',
